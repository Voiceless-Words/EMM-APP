		...
		if(navigator.onLine){
			...
			storeRequest.onsuccess = function(event) {
				...
				while(i < myRecords.length)
				{
					...
					console.log(key4);
					$.ajax({
						url:"http://emmapp.openode.io/form_save",
						data:{
							jobNo: jobNum,
							form:stringF,
						},
					...
					$.ajax({
						type : "POST",
						url : "http://emmapp.openode.io/jobcard_save",
						data : stringD,
...
var selectedCable = -1;
	...
	$("#loginForm").submit(function (e) {
		...
		check_data(user,  "http://emmapp.openode.io/user/login");
    });
	...
	$("#registerForm").submit(function (e) {
		...
		console.log(user);
		check_data(user,  "http://emmapp.openode.io/user/register");
    });
    ...
    function submit_data(user, path)
    {
        ...
        $.ajax({
            ...
            success : function(data) {
				...
				if (path === "http://emmapp.openode.io/user/register")
	...
	function update_field( name, user, path)
    {
        $.ajax({
            type : "POST",
            url : "http://emmapp.openode.io/user/update",
            data :{
				user : user.username,
				value : user.password
			},
...
function userClosedCard(pos) {
	...
	$.ajax({
		type : "POST",
		url : "http://emmapp.openode.io/search/getonecard",
		data :{
